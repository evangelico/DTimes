...
public class SubscriptionAction extends BaseActionSupport {
	...
	public String index() {
		...
		try {
			...
			if (StringUtils.isNotBlank(nameFilter) && plainFilter > 0) {
				...
				totalSubscriptions = subscriptionBusiness.countTotalSubscriptionByPlainAndNominative(nameFilter,plainFilter);
			} else if (StringUtils.isNotBlank(nameFilter)) {
				subscriptions = subscriptionBusiness.findLikeNominative(nameFilter, page, size);
				totalSubscriptions = subscriptionBusiness.countTotalSubscriptionByNominative(nameFilter);
			} else if (plainFilter > 0) {
				subscriptions = subscriptionBusiness.findByPlain(plainFilter, page, size);
				totalSubscriptions = subscriptionBusiness.countTotalSubscriptionByPlain(plainFilter);
			} else {
				subscriptions = subscriptionBusiness.findAll(page, size);
				totalSubscriptions = subscriptionBusiness.countTotalSubscription();
			}
	...
	public String saveEdit() {
		...
		try {
			...
			// birthdayDate = sdf.format(subscription.getBirthdayDate());
			// registrationDate =
			// sdf.format(subscription.getRegistrationDate());
			// return "subscription.insert.error";
			// }
			...
			// if
			// (!fiscalCodeValidator.ControllaCF(subscription.getFiscalCode()))
			// {
